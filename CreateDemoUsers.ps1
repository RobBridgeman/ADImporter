...
#
# User properties
$ou = "OU=Servants,DC=vm,DC=upstream,DC=se"
											 # Which OU to create the user in
$initialPassword = "Password1"               # Initial password set for the user
$orgShortName = "VMU"                        # This is used to build a user's sAMAccountName
$dnsDomain = "vm.upstream.se"                # Domain is used for e-mail address and UPN
$company = "Emile ITGlue API Test"			 # Used for the user object's company attribute
...
# Other parameters
$userCount = 10                             # How many users to create
...
{
	$Fname = $firstNames[$fn].Firstname
	$Lname = $lastNames[$ln].Lastname
	...
	-AccountExpirationDate  ((Get-Date).AddDays(700)) `
	...
	$fn++; $fn > .\fn
	if ($fn -gt $firstNames.length) {
		$fn =  0
	}

    $ln++; $ln > .\ln
	if ($ln -gt $lastNames.length) {
		$ln =  0
	}
}
